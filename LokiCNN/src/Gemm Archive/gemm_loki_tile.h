                   ...
                   )
{
    ...
    // scratchpad_write(0, B_start);     // index 0 stores the address of B
    // scratchpad_write(1, A_start);     // index 1 stores the address of A
    // scratchpad_write(3, C_start);     // index 3 stores the address of AB
    // scratchpad_write(5, incRowC);   // integer of size 4 byte
    // scratchpad_write(2, B_end);
    // scratchpad_write(7, beta);     
    ...
    asm volatile (
      ...
      "ldadd r10, r28, 0x00 -> 1 \n"
      "ldadd r14, r28, 0x04 -> 10 \n"
      "ldadd r18, r28, 0x08 -> 11 \n"
      "ldadd r22, r28, 0x0c -> 12 \n"
      "addu r28, r28, r30 \n"
      "or r0, r2, r0 \n"
      "or r0, r3, r0 \n"
      "or r0, r4, r0 \n"
                   ...
                   )
{
    ...
    addr = loki_mem_address(0, core, CH_REGISTER_4, GROUPSIZE_8, false, false, false, false);  
    ...
    for (j=core; j<np; j+=MATRIX_NUM_CORE) {
        ...
        for (i=0; i<mp; i++) {
            ...
            if (mr==MR && nr==NR) {
                        ...
                        // fprintf(stderr, "\n");
                        // dgemm_micro_kernel(&B[j*kc*NR], &A[i*kc*MR], &B[(j+1)*kc*NR], 
                                           // &C[i*MR*incRowC+j*NR], incRowC*4, 1);
                        dgemm_micro_kernel();
         ...
         const void* data1)
{
    ...
    int mc, nc, kc;
    int i, j, l;
    for (j=tile; j<nb; j+=MATRIX_NUM_TILE) {
    ...
    /*
    if(core == 0){
      fprintf(stderr, "id is %d : %d \n", tile, core);
      // if(tile == 0){
      //   for(int i = 0; i < 1; i++){
      //     for(int j = 0; j < 128; j++){
      //       fprintf(stderr, "%d ", C[i*128+j]);
      //     }
      //     fprintf(stderr, "\n");
      //   }
      // }
      for(int t = 0; t < MATRIX_NUM_TILE; t++){
        if(tile == t){
...
dgemm_nn(int            m,
         ...
         // int            incColC
                  ...
                  data->incColC = 1;
                  data->cores = MATRIX_NUM_CORE*MATRIX_NUM_TILE;

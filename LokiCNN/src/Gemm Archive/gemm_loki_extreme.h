                   ...
                   )
{
    ...
    asm volatile (
      ...
      "4:"
      "scratchrdi r28, 2 \n"        // load the value of this iteration's limit
      "scratchrdi r29, 7 \n"        // load the value of overall limit
      "setlt.p r0, r28, r29 \n"     // compare if first limit has reached
      "scratchrdi r27, 6 \n"
      "psel.fetchr 0b, 5f \n"        //? potential source of packet miss
      "addu r28, r28, r27 \n"
                   ...
                   )
{
    ...
    scratchpad_write(1, A);
    scratchpad_write(4, &A[mp*kc*MR]);
    scratchpad_write(6, MATRIX_NUM_CORE*kc*NR*4);
    scratchpad_write(7, &B[np*kc*NR]);
    // for (j=core; j<np; j+=MATRIX_NUM_CORE) {
    // scratchpad_write(0, &B[j*kc*NR]);     // index 0 stores the address of B
    // scratchpad_write(2, &B[(j+1)*kc*NR]);
    // scratchpad_write(3, &C[j*NR]);
    scratchpad_write(0, &B[core*kc*NR]);
    scratchpad_write(2, &B[(core+1)*kc*NR]);
    scratchpad_write(3, &C[core*NR]);
    scratchpad_write(10, &C[core*NR]);
    scratchpad_write(8, NR*4*MATRIX_NUM_CORE);
    dgemm_micro_kernel();       
    // }
}

/*! \file convolutional_layer_direct.h
    \brief direct (NAIVE) version of convolutional layer, no extra memory requirement
*/
// Convolutional Layer --------------------------------------------------------
            ...
            for(int fx = x; fx < x + width; fx++) {
                ...
                for(int fy = y; fy < y + height; fy++, index++) {
                  ...
                  if(fy >= 0 && fx >=0 && fx < V_sx) {
                    a = add(a, multiply(f->w[index], get_vol(V, fx, fy, fd)));
                  }
              }
            }
          }
...
conv_layer_t* make_conv_layer(int in_sx, int in_sy, int in_depth,
  ...
  for (int i = 0; i < filters; i++) {
    l->filters[i] = make_vol(l->sx, l->sy, l->in_depth, readDouble(0.0));
  }
    ...
    for (int z = 0; z < depth; z++)
      for (int x = 0; x < sx; x++)
        for (int y = 0; y < sy; y++){
          ...
          assert(items_read == 1);
          set_vol(l->filters[d], x, y, z, readDouble(val));
        }

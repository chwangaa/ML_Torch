                   ...
                   )
{
    ...
    for (j=core; j<np; j+=MATRIX_NUM_CORE) {
        ...
        // this is needed to make sure that correct number of cores are linked by core0
        if(core == 0){
          if(_np > MATRIX_NUM_CORE){
            channel_t address = loki_mcast_address(all_cores_except_current(MATRIX_NUM_CORE), CH_REGISTER_4 , false);
            _np -= MATRIX_NUM_CORE;
            set_channel_map(14, address);
          }
          else{
            if(_np != 1){
              channel_t address = loki_mcast_address(all_cores_except_current(_np), CH_REGISTER_4 , false);
              set_channel_map(14, address);
            }
            else{
              marker = 1;
         ...
         const void* data1)
{
    ...
    */
    int core = get_core_id();
    int addr = loki_mem_address(0, core, CH_REGISTER_2, GROUPSIZE_8, false, false, false, false);  

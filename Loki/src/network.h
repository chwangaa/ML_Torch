...
void net_forward_from_layer_num(Network* net, int i){
    Layer **layers = net->layers;
    Layer *layer;
    int num_layers = net->num_layers;
    assert(i < num_layers);
    for(int j = i; j < num_layers; j++){
        layer = layers[j];
        (*layer->forward)(layer, net->buffer[j], net->buffer[j+1], 0, 0);
    }
}

void net_forward_on_layer_num(Network* net, int i){
    Layer **layers = net->layers;
    Layer *layer;
    int num_layers = net->num_layers;
    assert(i < num_layers);
...
void net_measure_inference_time(Network* net){
    
    int num_layers = net->num_layers;
    for (int i = 0; i < num_layers; i++) {
        // copy_vol(net->buffer[0][0], input[i]);    // everytime, set the input at data_layer
        uint64_t start_time = timestamp_us();
        net_forward_on_layer_num(net, i);
        uint64_t end_time = timestamp_us();
        double dt = (double)(end_time-start_time) / 1000.0;
        fprintf(stderr, "time spent on layer %d is %lf \n", i, dt);
    }
}

...
void conv_forward_without_padding(conv_layer_t* l, vol_t** in, vol_t** out, int start, int end) {
  for (int i = start; i <= end; i++) {
    ...
    arr2_t* weights = make_arr2(weightx,weighty);
    arr2_t* data = make_arr2(datax,datay);

    for(int out_d=0; out_d<A_d; out_d++)
      for(int in_d=0; in_d<V_d; in_d++)
        for(int y=0; y<ky; y++)
          for(int x=0; x<kx; x++) {
            set_arr2(weights,(in_d*ky + y)*kx + x,out_d,
    ...
    for(int ay=0; ay<outy; ay++)
      for(int ax=0; ax<outx; ax++)
        for(int in_d=0; in_d<V_d; in_d++)
          for(int y=0; y<ky; y++)
            for(int x=0; x<kx; x++) {
              set_arr2(data,ay*outx + ax,(in_d*ky + y)*kx + x,
                      get_vol(V,x+ax*xy_stride,y+ay*xy_stride,in_d));
            }

    arr2_t *temp = strassen(weights,data);

    for(int out_d=0; out_d<A_d; out_d++)
      for(int ay=0; ay<outy; ay++)
        for(int ax=0; ax<outx; ax++)
          set_vol(A,ax,ay,out_d,get_arr2(temp,ay*outx+ax,out_d));

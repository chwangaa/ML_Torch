...
static const char* DATA_FOLDER = "data/cifar/";
...
network_t* load_cnn_snapshot() {
  ...
  fprintf(stderr, "Network loaded, start loading parameter \n");
  conv_load(net->l0, "models/cifar/layer1_conv.txt");
  conv_load(net->l3, "models/cifar/layer4_conv.txt");
  conv_load(net->l6, "models/cifar/layer7_conv.txt");
  fc_load(net->l9, "models/cifar/layer10_fc.txt");
  return net;
}
...
vol_t** load_batch(int batch) {
  ...
  char fn[1024];
  sprintf(fn, "%sdata_batch_%d.bin", DATA_FOLDER, batch+1);
  FILE* fin = fopen(fn, "rb");
  assert(fin != NULL);
...
double run_cifar10_classification(int* samples, int n, double** keep_output) {
